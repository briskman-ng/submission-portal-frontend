...
export default function AdminDashboardPage() {
  ...
  const stats = [
    ...
    { 
      label: 'New Today', 
      value: mockDashboardStats.newSubmissions, 
      icon: Clock, 
      color: 'bg-amber-500',
      change: '+3',
      changeType: 'up'
    ...
    <div className="min-h-screen">
      ...
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          {stats.map((stat, i) => (
            <div key={i} className="bg-white rounded-xl p-6 border border-stone-200 shadow-sm">
              ...
              <p className="font-display text-3xl font-bold text-stone-900 mt-4">{stat.value}</p>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-white rounded-xl border border-stone-200 shadow-sm">
            <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200">
              <h2 className="font-semibold text-stone-800">Recent Submissions</h2>
              <Link 
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${getPriorityColor(submission.priority)}`}>
                        {submission.priority}
                      </span>
                    </div>
                    <p className="text-sm text-stone-800 font-medium truncate">{submission.subject}</p>
          ...
          <div className="bg-white rounded-xl border border-stone-200 shadow-sm">
            <div className="px-6 py-4 border-b border-stone-200">
              <h2 className="font-semibold text-stone-800">Submissions by Status</h2>
            </div>
            <div className="p-6 space-y-4">
              {statusOptions.slice(0, 6).map((status) => {
                const count = mockSubmissions.filter(s => s.status === status.value).length;
        ...
        <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'New Submissions', count: 12, href: '/admin/submissions?status=new', color: 'border-blue-500' },
            { label: 'Urgent Priority', count: 3, href: '/admin/submissions?priority=urgent', color: 'border-red-500' },
            { label: 'Awaiting Response', count: 8, href: '/admin/submissions?status=in-review', color: 'border-amber-500' },
            { label: 'This Week', count: 47, href: '/admin/submissions?range=week', color: 'border-emerald-500' },
          ].map((action, i) => (
            <Link
              key={i}
              href={action.href}
            ...
            >
              <p className="font-display text-2xl font-bold text-stone-800">{action.count}</p>

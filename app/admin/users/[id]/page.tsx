...
import { mockAdminUsers, mockActivityLogs, mockSubmissions } from '@/lib/admin-mock-data';
...
export default function UserDetailPage({ params }: PageProps) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-stone-800">User Not Found</h2>
          <Link href="/admin/users" className="mt-4 inline-flex items-center gap-2 text-emerald-600 hover:text-emerald-700">
  ...
  const userActivityLogs = mockActivityLogs.filter(log => log.userId === user.id);
  ...
  const allUserActivity = mockSubmissions.flatMap(submission => 
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden sticky top-24">
              ...
              <div className="bg-gradient-to-br from-emerald-600 to-emerald-700 px-6 py-8 text-center">
                ...
                <span className={`inline-flex px-3 py-1 rounded-full text-xs font-medium mt-2 ${getRoleBadgeColor(user.role)}`}>
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-stone-700">
                    Status: <span className={`font-medium ${user.status === 'active' ? 'text-emerald-600' : 'text-red-600'}`}>
              ...
              <div className="border-t border-stone-200 p-6">
                <h3 className="text-sm font-semibold text-stone-800 mb-4">Activity Summary</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-stone-50 rounded-lg p-3 text-center">
                    <p className="text-2xl font-bold text-stone-800">{allUserActivity.length}</p>
                    <p className="text-xs text-stone-500">Total Actions</p>
                  </div>
                  <div className="bg-stone-50 rounded-lg p-3 text-center">
                    <p className="text-2xl font-bold text-stone-800">{userSubmissions.length}</p>
                    <p className="text-xs text-stone-500">Submissions Handled</p>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="divide-y divide-stone-100">
                {filteredActivity.length === 0 ? (
                  ...
                  filteredActivity.map((activity) => {
                    ...
                    return (
                      <div key={`${activity.id}-${activity.submissionId}`} className="px-6 py-4 hover:bg-stone-50 transition-colors">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <p className="text-sm text-stone-800">{activity.description}</p>
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <span className="text-xs text-stone-500 truncate">{activity.subject}</span>
                            </div>
                          </div>

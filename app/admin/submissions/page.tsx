...
export default function SubmissionsPage() {
  ...
  const getStatusColor = (status: string) => {
    return statusOptions.find(s => s.value === status)?.color || 'bg-stone-100 text-stone-800';
  };

  const getPriorityColor = (priority: string) => {
    return priorityOptions.find(p => p.value === priority)?.color || 'bg-stone-100 text-stone-800';
  };
  ...
  const hasActiveFilters = filters.type || filters.status || filters.priority || filters.dateRange !== 'all';
              ...
              >
                ...
                {hasActiveFilters && (
                  <span className="w-5 h-5 bg-emerald-600 text-white text-xs rounded-full flex items-center justify-center">
                    {[filters.type, filters.status, filters.priority].filter(Boolean).length}
                  </span>
                )}
            ...
            {showFilters && (
              <div className="mt-4 pt-4 border-t border-stone-200">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                  {/* Type Filter */}
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">Submission Type</label>
                    <select
                      value={filters.type}
                  ...
                  {/* Custom Date Range */}
                  <div>
                    <label className="block text-sm font-medium text-stone-700 mb-1">Custom Date Range</label>
                    <div className="flex items-center gap-2">
                      <input
                        type="date"
        ...
        <div className="bg-white rounded-xl border border-stone-200 shadow-sm overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-stone-200">
            <div className="flex items-center gap-4">
              ...
              >
                ...
                Sort by: {sortBy === 'date' ? 'Date' : sortBy === 'priority' ? 'Priority' : 'Status'}
              </button>
            </div>
            <div className="flex items-center gap-2">
              ...
              <tbody className="divide-y divide-stone-100">
                {filteredSubmissions.map((submission) => (
                  <tr key={submission.id} className="hover:bg-stone-50 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div>
                        <p className="text-sm font-medium text-stone-800">{submission.submitterName}</p>
                        <p className="text-xs text-stone-500">{submission.submitterEmail}</p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-sm text-stone-600">{getTypeLabel(submission.type)}</span>
                    </td>
                    <td className="px-6 py-4 max-w-xs">
                      <p className="text-sm text-stone-800 truncate">{submission.subject}</p>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium border ${getPriorityColor(submission.priority)}`}>
                        {submission.priority.charAt(0).toUpperCase() + submission.priority.slice(1)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusColor(submission.status)}`}>
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-xs text-stone-400">
                        {new Date(submission.submittedAt).toLocaleTimeString('en-NG', {
                          hour: '2-digit',
                          minute: '2-digit'
                        })}
                      </div>
          ...
          {filteredSubmissions.length === 0 && (
            <div className="py-12 text-center">
              ...
              <p className="text-stone-500 font-medium">No submissions found</p>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-stone-200 bg-stone-50">
            <p className="text-sm text-stone-500">
              Showing <span className="font-medium">{filteredSubmissions.length}</span> of <span className="font-medium">{mockSubmissions.length}</span> submissions
            </p>
            <div className="flex items-center gap-2">
              <button className="px-3 py-1.5 text-sm text-stone-600 hover:bg-white rounded border border-stone-300 disabled:opacity-50" disabled>
                Previous
              </button>
              <button className="px-3 py-1.5 text-sm text-white bg-emerald-700 rounded">1</button>
              <button className="px-3 py-1.5 text-sm text-stone-600 hover:bg-white rounded border border-stone-300">2</button>

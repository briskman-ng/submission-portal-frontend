...
import { Search, Loader2, Download, CheckCircle, RefreshCw } from 'lucide-react';
...
function TrackingContent() {
  ...
  const timeline = [
    { status: 'Submitted', date: 'Jan 29, 2026 • 10:34 AM', description: 'Submission received and logged into system', completed: true },
    { status: 'Under Review', date: 'Jan 29, 2026 • 2:15 PM', description: 'Assigned to Projects Department for initial review', completed: true, current: true },
    { status: 'Processing', date: 'Pending', description: 'Detailed assessment and verification', completed: false },
    { status: 'Decision', date: 'Pending', description: 'Final decision and response preparation', completed: false },
    { status: 'Completed', date: 'Pending', description: 'Resolution communicated to submitter', completed: false },
  ];
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-emerald-900/10 border border-stone-200 overflow-hidden mb-8">
          <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 px-6 py-6 text-center">
            ...
            <h1 className="font-display text-xl font-semibold text-white">Track Your Submission</h1>
            <p className="text-emerald-200 text-sm mt-1">Enter your tracking ID to view the current status</p>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl shadow-emerald-900/10 border border-stone-200 overflow-hidden animate-fade-in-up">
            ...
            <div className="px-6 py-5 border-b border-stone-200 flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <span className="font-mono text-lg font-bold text-emerald-700">{trackingId || 'NDDC-2024-00847'}</span>
                  <StatusBadge status="in-review" />
                </div>
                <p className="text-stone-600 text-sm">Rural Electrification Project - Bayelsa State</p>
              </div>
            ...
            <div className="p-6">
              {activeTab === 'status' && (
                <>
                  {/* Timeline */}
                  <h3 className="font-semibold text-stone-800 mb-6">Submission Timeline</h3>
                    ...
                    {timeline.map((item, i) => (
                      <div key={i} className="flex gap-4 pb-8 last:pb-0">
                        <div className="flex flex-col items-center">
                          ...
                          {i < timeline.length - 1 && (
                            <div className={`w-0.5 flex-1 mt-2 ${item.completed ? 'bg-emerald-300' : 'bg-stone-200'}`} />
                          )}
                        </div>
                        <div className={`flex-1 ${item.current ? 'bg-emerald-50 -mx-3 px-3 py-2 rounded-lg border border-emerald-100' : ''}`}>
                          <div className="flex items-center justify-between">
                            <p className={`font-medium ${item.completed ? 'text-stone-800' : 'text-stone-400'}`}>{item.status}</p>
                            <span className={`text-xs ${item.completed ? 'text-stone-500' : 'text-stone-300'}`}>{item.date}</span>
                          </div>
                          <p className={`text-sm mt-1 ${item.completed ? 'text-stone-600' : 'text-stone-400'}`}>{item.description}</p>
                          {item.current && (
                  ...
                  <div className="px-6 pb-6">
                    <div className="bg-stone-50 rounded-xl p-4 grid md:grid-cols-3 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium text-stone-800">Projects Directorate</p>
                      </div>
                      <div>
                        <p className="text-stone-500">Expected Response</p>
                        <p className="font-medium text-stone-800">Within 5 business days</p>
                      </div>
                      <div>
                        <p className="text-stone-500">Last Updated</p>
                        <p className="font-medium text-stone-800">2 hours ago</p>
                      </div>
                    </div>
                  </div>
                </>
              ...
              {activeTab === 'description' && (
                <div>
                  <h3 className="font-semibold text-stone-800 mb-4">Submission Description</h3>
        ...
        {!showResult && (
          <div className="text-center text-stone-500 text-sm">
            <p>Can&apos;t find your tracking ID? Check your email confirmation or{' '}
